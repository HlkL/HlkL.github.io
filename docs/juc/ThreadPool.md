...
`java.util.concurrent.BlockingQueue` 接口阻塞队列实现：
...
- 当一个节点入队：
  ...
  ![img](https://camo.githubusercontent.)
...
- `h = head` → `first = h.next`
  ...
  ![img](https://camo.githubusercontent.)

- `h.next = h` → `head = first`

  ![img](https://camo.githubusercontent.)
...
- handler：拒绝策略，线程到达最大线程数仍有新任务时会执行拒绝策略，`RejectedExecutionHandler` 下有 4 个实现类：
...
![img](https://camo.githubusercontent.)
...
![img](https://camo.githubusercontent.com/03bae416acbe3252afa2c2926cbd980a325ff480070036e49108035fe06dac92/68747470733a2f2f7365617a65616e2e6f73732d636e2d6265696a696e672e616c6979756e63732e636f6d2f696d672f4a6176612f4a55432d6e657753696e676c655468726561644578656375746f722e706e67)
...
- 状态表示：
  ...
  ![image-20240414183219984](https://hougen.oss-cn-guangzhou.aliyuncs.com/blog-img/1713090740-image-20240414183219984.png)
...
- submit()：**把 Runnable 或 Callable 任务封装成 FutureTask 执行**，可以通过方法返回的任务对象，调用 get 阻塞获取任务执行的结果或者异常
...
- addWorker()：**添加线程到线程池**，返回 true 表示创建 Worker 成功，且线程启动。首先判断线程池是否允许添加线程，允许就让线程数量 + 1，然后去创建 Worker 加入线程池
  ...
  **<font color='red'>注意：</font>** SHUTDOWN 状态也能添加线程，但是要求新加的 Woker 没有 firstTask，而且当前 queue 不为空，所以创建一个线程来帮助线程池执行队列中的任务
...
FutureTask 继承 Runnable、Future 接口，用于包装 Callable 对象，实现任务的提交
...
- 定时任务：
  ...
  - `scheduleAtFixedRate`：**一次任务的启动到下一次任务的启动**之间只要大于等于间隔时间，抢占到 CPU 就会立即执行
...
> ScheduledFutureTask 继承 FutureTask，实现 RunnableScheduledFuture 接口，具有延迟执行的特点，覆盖 FutureTask 的 run 方法来实现对**延时执行、周期执行**的支持。对于延时任务调用 FutureTask#run，而对于周期性任务则调用 FutureTask#runAndReset 并且在成功之后根据 fixed-delay/fixed-rate 模式来设置下次执行时间并重新将任务塞到工作队列，在调度线程池中无论是 runnable 还是 callable，无论是否需要延迟和定时，所有的任务都会被封装成 ScheduledFutureTask
...
- 执行时间：
  ...
  - fixed-rate：两次开始启动的间隔，

  - fixed-delay：一次执行结束到下一次开始启动
...
- `runAndReset()`：周期任务正常完成后**任务的状态不会变化**，依旧是 NEW，不会设置 outcome 属性。但是如果本次任务执行出现异常，会进入 setException 方法将任务状态置为异常，把异常保存在 outcome 中，方法返回 false，后续的该任务将不会再周期的执行

- `setNextRunTime()`: 任务下一次的触发时间
...
> DelayedWorkQueue 是支持延时获取元素的阻塞队列，内部采用优先队列 PriorityQueue（小根堆、满二叉树）存储元素，其他阻塞队列存储节点的数据结构大都是链表，**延迟队列是数组**，所以延迟队列出队头元素后需要**让其他元素（尾）替换到头节点**，防止空指针异常
...
- `siftup()`：插入新节点后对堆进行调整，进行节点上移，保持其特性【节点的值小于子节点的值】，小顶堆
...
- schedule()：延迟执行方法，并指定执行的时间，默认是当前时间
  ...
  **<font color='red'>overflowFree的原因：</font>** 如果某个任务的 delay 为负数，说明当前可以执行（其实早该执行了）。阻塞队列中维护任务顺序是基于 compareTo 比较的，比较两个任务的顺序会用 time 相减。那么可能出现一个 delay 为正数减去另一个为负数的 delay，结果上溢为负数，则会导致 compareTo 产生错误的结果
...
Fork/Join：线程池的实现，体现是分治思想，适用于能够进行任务拆分的 CPU 密集型运算，用于**并行计算**，**Fork/Join 在**分治的基础上加入了多线程，把每个任务的分解和合并交给不同的线程来完成，提升了运算效率
